defmodule Moonwalk.Spec do
  ...
  defp put(container, key, value) when is_list(container) do
    Keyword.put(container, key, value)
  end

  defp put(container, key, value) when is_map(container) do
    Map.put(container, key, value)
  end

  def rename_input({target, input, output}, inkey, outkey) do
    case pop(input, inkey) do
      {:ok, value, input} -> {target, put(input, outkey, value), output}
